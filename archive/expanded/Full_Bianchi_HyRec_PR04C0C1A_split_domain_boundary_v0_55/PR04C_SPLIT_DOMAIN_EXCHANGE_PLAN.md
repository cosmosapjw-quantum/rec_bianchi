...
Phi_Egamma^s transported photon energy per H per second,
Phi_EH^s     optional *physical collision* atom-energy source, not an interface default,
...
Phi_EH^s = 0 for a pure computational representation crossing
```

up to roundoff. A computational interface is not a new atom-photon event, so
absolute photon energy crossing it must not be paired with a second atom recoil
source. Atom energy and spatial momentum are retained only where an existing
physical same-event collision four-force determines them; neither may be
inferred from a scalar frequency moment alone. This v0.55 correction supersedes
the preliminary `Phi_Egamma+Phi_EH=0` interface wording.
...
- exact opposite-sign number/transported-photon-energy application;
- zero atom source for a pure representation crossing;
