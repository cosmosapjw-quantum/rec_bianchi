...
def test_fixed_step_limit_matches_one_backward_euler_step() -> None:
    history = _history()
    context = trajectory.AdaptiveTrajectoryContext(
        eta=history.grid.eta[-1],
    ...
    expected = _linear_step(context.state_vector, history.grid.dlna).state_vector
    assert ledger.accepted_microsteps == 1
    assert ledger.rejected_microsteps == 0
    assert np.array_equal(updated.state_vector, expected)
