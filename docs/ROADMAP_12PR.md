...
5. **Primitive HYREC operator and BackgroundSnapshot/RadiationFeedback trajectory interface** — **IN PROGRESS**.
   ...
   - **PR-05B3 NEXT:** install an XOR owner transition for scalar `Dfplus`/`Dfplus_Ly` feedback and assemble the coupled accepted-step residual/Jacobian. Sobolev, native `A1s` diffusion and completed/Schur `Tvv` remain canonical until independently complete replacements exist.
   - **PR-05C:** short adaptive trajectory with accepted-step history commit, event-localized interface/stencil crossings, solver restart and timestep refinement. No FLRW history-parity claim yet.
